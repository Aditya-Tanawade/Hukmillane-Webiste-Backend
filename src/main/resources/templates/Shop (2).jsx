...
export default function Shop() {
  ...
  const [receiptData, setReceiptData] = useState(null);   // success
  const [failureData, setFailureData] = useState(null);   // failure
...
function OrderPopup({ product, onClose, onSuccess, onFailure }) {
  ...
  function handlePhoneChange(val) {
    ...
    setPhoneError(digits.length > 0 ? validatePhone(digits) : null);
  }

  async function handlePay(e) {
    e.preventDefault();

    const pErr = validatePhone(customer.phoneNumber);
    if (pErr) { setPhoneError(pErr); return; }
    if (!customer.name.trim()) { setInlineError("Full name is required."); return; }
    ...
    try {
      ...
      // Step 1: Create order in backend (status = CREATED)
      ...
      try {
        ...
        // Payment failed / dismissed — call callback with failure info if available
        if (rzpErr && rzpErr.metadata) {
          try {
      ...
      // Step 3: Verify with backend via paymentCallback
      const verified = await paymentCallback({
        razorpay_order_id: payment.razorpay_order_id,
        razorpay_payment_id: payment.razorpay_payment_id,
        razorpay_signature: payment.razorpay_signature,
      });

      if (verified && verified.orderStatus === "PAYMENT_SUCESS") {
        onSuccess({ ...verified, customerName: customer.name, productName: product.name });
      } else {
        onFailure({ description: "Payment verification failed. Contact support.", orderId: order.razorpayOrderId });
      }
    } catch (err) {
        ...
        {step === 2 && (
          <div style={s.popupBody}>
            ...
            <form onSubmit={handlePay} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <div style={{ position: "relative" }}>
                  ...
                  <input
                    ...
                    style={{ ...s.input, paddingLeft: 44, borderColor: phoneError ? "#ef4444" : customer.phoneNumber.length === 10 ? "#22c55e" : "#f5deb3" }}
                    maxLength={10}
                  />
                  {customer.phoneNumber.length === 10 && !phoneError && (
                    <span style={s.phoneCheck}>✓</span>
                  )}
                </div>
                {phoneError && <span style={s.fieldError}>{phoneError}</span>}
                {!phoneError && customer.phoneNumber.length > 0 && customer.phoneNumber.length < 10 && (
                  <span style={s.fieldHint}>{10 - customer.phoneNumber.length} more digits needed</span>
                )}
              </div>
              ...
              {inlineError && (
                <div style={s.inlineError}>{inlineError}</div>
              )}

              <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                <button type="button" onClick={() => { setStep(1); setInlineError(null); }} style={s.backBtn}>← Back</button>
                <button type="submit" disabled={loading || !!phoneError} style={{ ...s.primaryBtn, flex: 1, opacity: loading || !!phoneError ? 0.7 : 1 }}>
...
function ReceiptPopup({ data, onClose }) {
  ...
  function handleDownload() {
    // Build a plain HTML receipt and trigger print-to-PDF
    const receiptHtml = `
<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8"/>
  <title>Receipt – ${data.razorpayOrderId || "Order"}</title>
  <style>
    ...
    .header h1 { color: #b71c1c; font-size: 22px; margin: 0 0 4px; }
    .header p { color: #757575; font-size: 13px; margin: 0; }
    ...
    .row .value { font-weight: 700; color: #212121; }
    ...
    .sizes { background: #fff9f0; border-radius: 8px; padding: 12px 14px; margin: 12px 0; }
    .sizes p { margin: 4px 0; font-size: 13px; font-weight: 600; }
    .footer { text-align: center; margin-top: 32px; font-size: 12px; color: #bdbdbd; }
  </style>
</head>
<body>
  <div class="header">
    <h1>🎉 HukmillLane Cha Raja</h1>
  ...
  <div class="row"><span class="label">Order ID</span><span class="value">${data.razorpayOrderId || "—"}</span></div>
  <div class="row"><span class="label">Payment ID</span><span class="value">${data.razorpayPaymentId || "—"}</span></div>
  ...
  ${data.sizeQuantities && data.sizeQuantities.length > 0
    ...
    <div style={s.overlay}>
      <div style={{ ...s.popup, maxWidth: 460 }}>
        ...
        <div style={s.receiptBody} ref={receiptRef}>
          {/* Dashed divider with RECEIPT label */}
            ...
            <div style={s.sizesBox}>
              {data.sizeQuantities.map((sq) => (
                <div key={sq.size} style={{ display: "flex", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 13, color: "#616161", fontWeight: 600 }}>Size {sq.size} × {sq.quantity}</span>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#424242" }}>{money(sq.quantity * (data.amount / data.totalQuantity))}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ padding: "0 22px 24px", display: "flex", gap: 10 }}>
          ...
          <button onClick={onClose} style={{ ...s.primaryBtn, flex: 1 }}>
            Done
          </button>
        </div>
      </div>
      ...
      <div style={{ ...s.popup, maxWidth: 420 }}>
        ...
        <div style={s.failureBody}>
          ...
          <div style={s.failureTips}>
            ...
            {["Ensure you have sufficient balance.", "Try a different payment method.", "Contact your bank if the issue persists."].map((tip) => (
        ...
        <div style={{ padding: "0 22px 24px", display: "flex", gap: 10 }}>
          ...
          <button onClick={onRetry} style={{ ...s.primaryBtn, flex: 1, background: "#b71c1c" }}>
            Try Again
          </button>
        </div>
      </div>
...
const s = {
  ...
  closeBtn: {
    ...
    borderRadius: "50%", cursor: "pointer", fontSize: 13, fontWeight: 700, color: "#757575",
    display: "flex", alignItems: "center", justifyContent: "center",
  },
  ...
  sizeSelect: {
    ...
    padding: "9px 12px", fontSize: 14, fontWeight: 700, color: "#424242",
    background: "#fffdf7", cursor: "pointer", outline: "none",
  },
  stepper: {
    display: "flex", alignItems: "center", gap: 2, background: "#fafafa",
    borderRadius: 10, border: "1.5px solid #f5deb3", padding: "2px 4px",
  },
  stepBtn: {
  ...
  orderSummary: {
    background: "#fff9f0", borderRadius: 12, padding: "14px 16px",
    border: "1px solid #f5deb3", display: "flex", flexDirection: "column", gap: 6,
  },
  summaryLine: { display: "flex", justifyContent: "space-between", fontSize: 14 },
  // Phone
  phonePrefix: {
    position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)",
    fontWeight: 800, fontSize: 14, color: "#616161", pointerEvents: "none",
  },
  phoneCheck: {
    position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)",
    color: "#22c55e", fontWeight: 900, fontSize: 16,
  },
  ...
  backBtn: {
    ...
    borderRadius: 50, padding: "13px 18px", fontSize: 14, fontWeight: 700, cursor: "pointer",
  },

  // ── Success / Receipt ──
  successHeader: {
    background: "linear-gradient(135deg,#052e16,#14532d)",
    padding: "32px 24px 28px", textAlign: "center", borderRadius: "22px 22px 0 0",
  },
  successIcon: {
  ...
  sizesBox: {
    background: "#fff9f0", border: "1px solid #f5deb3",
    borderRadius: 10, padding: "10px 14px",
    display: "flex", flexDirection: "column", gap: 4, margin: "4px 0",
  },
  receiptTotal: {
    display: "flex", justifyContent: "space-between",
    fontSize: 20, fontWeight: 900, padding: "14px 0 4px",
  },
  downloadBtn: {
    background: "#f0fdf4", color: "#166534", border: "1.5px solid #bbf7d0",
    borderRadius: 50, padding: "13px 18px", fontSize: 14, fontWeight: 800,
