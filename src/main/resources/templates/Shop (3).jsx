...
// Builds a compact size+qty string for Razorpay notes, e.g. "Size 28 x2, Size 32 x1"
...
function OrderPopup({ product, onClose, onSuccess, onFailure }) {
  ...
  // Touched flags — only show errors after user has interacted with the field
  ...
  async function handlePay(e) {
    ...
    try {
      ...
      const sizeNote = buildSizeNote(rows);
      ...
      // 1. Create order in backend (status = CREATED)
      const order = await createOrder(payload);

      // 2. Open Razorpay with notes containing size/qty breakdown
      let payment;
      try {
          ...
          <div style={s.popupBody}>
            ...
            <form onSubmit={handlePay} style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <ValidatedInput
                ...
                error={touched.phoneNumber ? phoneError : null}
                valid={phoneValid}
                hint={phoneHint}
              />
...
function ReceiptPopup({ data, onClose }) {
  ...
  function handleDownload() {
    const sizeRows = data.sizeQuantities && data.sizeQuantities.length > 0
              ...
              {data.sizeQuantities.map((sq) => (
                <div key={sq.size} style={{ display: "flex", justifyContent: "space-between" }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#424242" }}>
                    {money(sq.quantity * (data.amount / data.totalQuantity))}
                  </span>
                </div>
              ))}
            </div>
